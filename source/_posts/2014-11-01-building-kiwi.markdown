...
* Kiwi takes collected data that is either in NIPO or TripleS format and transfers it to a SQL database in relational format and makes analysis & monitoring a lot easier. Apart from the data, Kiwi will also create question references in the target SQL database.
...
* For these reasons Kiwi can be used by any market research company that is utilizing data collection platforms built by NIPO Software or any other platform that supports TripleS standard.
